...
def outPutFingerStyle(piece, dancerLimits=20):
    ...
    for ChordNotes in piece:
        arrangedChordNotes = arrangeLists(ChordNotes)
        try:
            currentDancers = copy.deepcopy(AllDancers)
            DancerAmount = len(currentDancers)
            for dancer in currentDancers:
                NewDancers = dancerMaker(dancer, arrangedChordNotes, dancerLimits)
            ...
            if len(AllDancers) > DancerAmount + dancerLimits:
                ...
                if len(AllDancers) == 1:
                    print('这个和弦只有唯一解了：')
                    print(arrangedChordNotes)
                if len(AllDancers) == 0:
                    print('这个和弦无解了：')
                    print(arrangedChordNotes)
                    currentDancers[0].outPutNote()
                    raise
        except:
            print('这个和弦弹不了：')
            print(arrangedChordNotes)
...
if __name__ == '__main__':
    # midiFile = 'guitarMidiFlies/Aguado_Polonaise_op2no2.mid'
